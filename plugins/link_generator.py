...
async def batch(client: Client, message: Message):
    ...
    while True:
        ...
        if s_msg_id:
            ...
            await second_message.reply("❌ SALAH\n\nPostingan yang Diteruskan ini bukan dari Saluran DB saya atau Tautan ini diambil dari Saluran DB", quote = True)
            continue
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔁 𝔹𝔸𝔾𝕀𝕂𝔸ℕ", url=f'https://telegram.me/share/url?url={link}')]])
...
async def link_generator(client: Client, message: Message):
    while True:
        try:
            channel_message = await client.ask(text = "Teruskan Pesan dari Saluran DB (dengan Kutipan)..\natau Kirim tautan Kiriman Saluran DB", chat_id = message.from_user.id, filters=(filters.forwarded | (filters.text & ~filters.forwarded)), timeout=60)
        except:
            return
